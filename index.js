...
const firestore = new Firestore({
  projectId: config.opsProjectId,
  keyFilename: './cred.firestore.json'
})
...
async function writeLogAndReportsToStorage(obj, id) {
  ...
  const docPath = `${id}/report_${obj.lhr.fetchTime}`;
  log(`${id}: Writing log to bucket ${config.gcs.bucketName} on the path ${docPath}`);
...
/**
 * Fetch the client URLs from Firestore and prepare them to be processed
 *
 * @returns {Promise<array>} An array of client name, id and site URLs.
 */
async function getClientArray () {
  var clientList = await firestore.collection('clientList').where('url', '>', '').get()
    .then(res => {
...
async function launchLighthouse (event, callback) {
  try {
    var source = await getClientArray();
    ...
    const id = src.id;
    const url = src.url;
    const clientName = src.clientName;
    ...
    const docPath = await writeLogAndReportsToStorage(res, id);
    console.log(`docPath: ${docPath}`);
    const json = createJSON(res.lhr, id);

    json.job_id = uuid;
    json.docPath = docPath;
